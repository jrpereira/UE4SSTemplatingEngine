...
te:registerTemplate(path);te:loadTemplatesFromRegister()
assert(#te.registry.templates == 1 and te.registry.templates[1].template.widgetRenderingEnabled == false,
    'expected one rendering-disabled provider template')
local catalog
...
write('mod_settings.ini',menu.manifest)
...
write('menu-profile.lua','return {mode="menu-test",templates={"../QuickslotsForever/templates/default.lua"},description='..string.format('%q',description)..'}\n')
write('enabled.txt','')
print('Built '..#menu.rows..' settings from '..path)
