-- Adapt the copied menu schema to the fresh runtime's value-only settings.
local U = require('mct.util')
local Provider = require('mct.provider_settings')
local M = {}
local function isDeclaration(settings)
    return type(settings) == 'table' and (settings.fields ~= nil or settings.groups ~= nil or settings.target ~= nil)
end
...
function M.build(definitions, templates, locations)
    ...
    for _, definition in ipairs(definitions) do
        ...
        local legacy = not definition.menu and isDeclaration(definition.settings)
        local declaration = U.copy(definition.menu or (legacy and definition.settings) or {})
        local values = U.copy(not legacy and definition.settings or {})
        ...
        categories[definition.name] = {name=definition.name, settings=declaration, runtimeSettings=values,
    ...
    for index, template in ipairs(templates) do
        ...
        local legacy = not template.menu and isDeclaration(template.settings)
        local declaration = U.copy(template.menu or (legacy and template.settings) or {})
        ...
        local values = U.copy(not legacy and template.settings or {})
        overrideDefaults(declaration, values)
        ...
        local metadata = {name=template.name, category=template.category, settings=declaration}
