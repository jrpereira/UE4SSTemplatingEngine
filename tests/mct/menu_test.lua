...
local Model = require('mct.menu_model')
local Menu = require('mct.menu')
local Runtime = require('mct.runtime')
local Controller = require('mct.menu_controller')
local Files = require('mct.menu_files')
...
local function fixture(single)
    ...
    local function template(id, target)
        local t = {id=id, name=id, category=category.name, menu={enabled=true, target=target,
        ...
        for _, operation in ipairs({'attach','update','detach'}) do
            t[operation] = function(object, settings)
                calls[#calls+1] = {id=id,operation=operation,settings=U.copy(settings),object=object}
            end
        end
        return t
    end
    local a,b = template('Alpha','templates'), template('Beta','module')
    local locations = {'/Mods/Alpha/ModCore/templates/template.lua','/Mods/Beta/ModCore/templates/template.lua'}
...
test('existing field declaration shape is accepted without leaking metadata into values', function()
    local declaration={enabled=true,target='templates',groups={{id='Layout',label='Layout'}},fields={field('Size',42)}}
    local template={id='old-format',name='Old format',category='player.quickslots',settings=declaration,
        attach=function() end,update=function() end,detach=function() end}
    local model=Model.build({{name='player.quickslots'}},{template},{'fixture.lua'})
    assert(template.settings.Size==42 and template.settings.fields==nil)
    assert(model.registry.templates[1].template.settings.fields[1].id=='Size')
end)
...
test('generated data goes into ModCore/cache and DMM resolves its nested config', function()
    local f=fixture(true)
    ...
    for _,row in ipairs(f.menu.rows) do
        if row.mcNavigation~=1 then assert(row.ConfigFile=='ModCore/cache/config.ini') end
    end
...
test('legacy generated files migrate without changing contents', function()
    local root=testRoot..'/migration'
    local paths=Layout.prepare(root)
    for _,name in ipairs({'config.ini','identity-catalog.lua','menu-pages.lua'}) do
        local out=assert(io.open(root..'/'..name,'w')); out:write('preserve '..name); out:close()
    end
    Layout.prepare(root)
    for _,name in ipairs({'config.ini','identity-catalog.lua','menu-pages.lua'}) do
        assert(io.open(root..'/'..name)==nil)
        local input=assert(io.open(paths.cache..'/'..name)); local contents=input:read('*a'); input:close()
        assert(contents=='preserve '..name)
    end
    Layout.prepare(root) -- migration is idempotent
end)

test('migration checks all conflicts before moving any existing file', function()
    local root=testRoot..'/conflicting-layout'
    local paths=Layout.prepare(root)
    for _,path in ipairs({root..'/config.ini',root..'/identity-catalog.lua',paths.catalog}) do
        local out=assert(io.open(path,'w')); out:write(path); out:close()
    end
    assert(not pcall(Layout.prepare,root))
    for _,path in ipairs({root..'/config.ini',root..'/identity-catalog.lua',paths.catalog}) do
        local input=assert(io.open(path)); local contents=input:read('*a'); input:close(); assert(contents==path)
    end
    assert(io.open(paths.config)==nil)
end)
