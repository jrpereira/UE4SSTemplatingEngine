...
local Source=require('mct.widget_source')
...
local lifetime={version=1}; local tokens={}; local nextToken=0
function lifetime.capture(address)
    if not tokens[address] then nextToken=nextToken+1; tokens[address]=tostring(nextToken) end
    return tokens[address]
end
function lifetime.valid(address,token) return tokens[address]==token end
function lifetime.takeLost() end
...
local api={MCTNative={lifetimes=lifetime}}
...
local host=References.new(source,lifetime)
local calls={}
local template={id='visual',category='quickslots'}
for _,name in ipairs({'attach','update','detach'}) do
    template[name]=function(o,settings) calls[#calls+1]={name,o,settings} end
end
...
assert(#calls==1 and calls[1][1]=='attach' and calls[1][2]==root and calls[1][3].size==2)
