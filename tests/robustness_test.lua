...
local categories = C.new(); categories:registerCategory('player', {'actions'})
local template = {collection = 'Robustness', name = 'Lifecycle', category = 'player.actions',
...
template.attach = function(_, _, _, previous)
    ...
    local nested, nestedWhy = runtime:detach('player.actions', {}, 'none')
    check(not nested and nestedWhy == 'reentrant lifecycle operation')
    return previous
end
check(runtime:apply('player.actions', identity, {}, {}))
check(runtime.revision == 0 and runtime.active['player.actions'].handle == original)
template.detach = function() return true end
check(runtime:detach('player.actions', {}, 'world_pre_unload'))
...
local selected = next(base.selectors['player.actions'].byValue)
local a, b = base.definitions['player.actions'][selected], reordered.definitions['player.actions'][selected]
...
defaults[base.selectors['player.actions'].id] = selected
...
check(base.decode(defaults)['player.actions'].configuration.groups.A.mode == 2)
