...
In group-first mode, TE maps `settings.shared` to `IA_SharedSlot*` actions and dispatches each slot through the selected group. Bridge callbacks must bind before TE activates its mapping contexts or gates native quickslot actions. Binding failure leaves native input available.
